            ...
            @foreach ($fuelTypes as $key => $item)
                <tr>
                    ...
                    <td>
                     ...
                     <td>
                        <img src="{{asset($item->second_image)}}" width="50px">
                     </td>
                    <td>
                        <a href="{{ route('admin.fuel_types.edit', $item->id) }}"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                  ...
                                  document.getElementById('delete-form-{{ $item->id }}').submit();"
                                            href="{{ route('admin.fuel_types.destroy', $item->id) }}"
                                            class="btn btn-danger text-white" data-toggle="modal"
                                            data-target="#colorDelete">Delete</a>
                                        <form id="delete-form-{{ $item->id }}"
                                            action="{{ route('admin.fuel_types.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @method('DELETE')
                                            @csrf
                                        </form>

<div class="table-responsive">
    <table class="table mb-0">
        ...
        <tbody>
            @foreach ($expertStatuses as $key => $item)
                <tr>
                    <th scope="row">{{ $key + 1 }}</th>
                    <td>{{ $item->status }}</td>
                    <td>
                        <a href="{{ route('admin.expert_status.edit', $item->id) }}"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Are You Sure You want to delete {{ $item->status }} Status?
                                    </div>
                                  ...
                                  document.getElementById('delete-form-{{ $item->id }}').submit();"
                                            href="{{ route('admin.expert_status.destroy', $item->id) }}"
                                            class="btn btn-danger text-white" data-toggle="modal"
                                            data-target="#colorDelete">Delete</a>
                                        <form id="delete-form-{{ $item->id }}"
                                            action="{{ route('admin.expert_status.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @method('DELETE')
                                            @csrf
                                        </form>

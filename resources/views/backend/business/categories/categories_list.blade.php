        ...
        <tbody>
            @foreach ($categories as $key => $item)
                <tr>
                    ...
                    <td><img src="{{ asset($item->category_image) }}" width="70px"></td>
                    <td>{{ $item->category_description }}</td>
                    <td>
                        <a href="{{ route('business.category.edit', $item->id) }}"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Are You Sure You want to delete {{ $item->category_name }} Type?
                                    </div>
                                  ...
                                  document.getElementById('delete-form-{{ $item->id }}').submit();"
                                            href="{{ route('business.category.destroy', $item->id) }}"
                                            class="btn btn-danger text-white" data-toggle="modal"
                                            data-target="#colorDelete">Delete</a>
                                        <form id="delete-form-{{ $item->id }}"
                                            action="{{ route('business.category.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @method('DELETE')
                                            @csrf
                                        </form>

        ...
        <tbody>
            @foreach ($wheels as $key => $item)
                <tr>
                    ...
                    <td>{{ $item->name }}</td>
                    <td>
                        <a href="{{ route('admin.wheels.edit', $item->id) }}"
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Are You Sure You want to delete {{ $item->name }} specification?
                                    </div>
                                  ...
                                  document.getElementById('delete-form-{{ $item->id }}').submit();"
                                            href="{{ route('admin.wheels.destroy', $item->id) }}"
                                            class="btn btn-danger text-white" data-toggle="modal"
                                            data-target="#colorDelete">Delete</a>
                                        <form id="delete-form-{{ $item->id }}"
                                            action="{{ route('admin.wheels.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @method('DELETE')
                                            @csrf
                                        </form>
